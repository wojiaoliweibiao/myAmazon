...
*/
(function($){
$.fn.upp_Huploadify = function(opts){
	...
	var myItemTemp = '<tr id="${fileID}" class="uploadify-queue-item">' +
                    '<td class="up-td" style="width:40%">${fileName}</td>'+
                    '<td class="up-td" style="width:30%">${fileSize}</td>'+
                    '<td class="up-td" style="width:20%"><div class="uploadify-progress"><div class="uploadify-progress-bar"></div></div></td>' +
                    '<td class="up-td" style="width:10%"><button type="button" class="btn btn-danger delfilebtn" style="color:#fff">删除</button></td></tr>';
    var defaults = {
		...
		saveInfoLocal:false,//用于开启断点续传模式，是否使用 localStorage 存储已上传文件大小
			  ...
			  if (xhr.upload && uploadedSize !== false) {
				  ...
				  xhr.onreadystatechange = function(e) {
					  if (xhr.readyState == 4) {
						  ...
						  if (fileObj.uploadOver) {
						  	option.onUploadComplete&&option.onUploadComplete(originalFile,xhr.responseText,option.modal_trigger);
							  //清除文件选择框中的已有值
//							  fileObj.fileInput.val('');
						  };
				...
				//option.formData['last_time'] = originalFile.lastModifiedDate.getTime();
				  ...
				  if("lastModifiedDate" in originalFile){
				  	option.formData.lastModifiedDate = originalFile.lastModifiedDate.getTime();
				  }else{
				  	option.formData.lastModifiedDate = originalFile.lastModified;
				  }
